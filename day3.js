...
const getCharValue = (chr) => {
    if (chr.trim() === '') return 0;
...
console.log(input.split('\n').map(row => {
    ...
    const part2 = row.slice(row.length/2);

    let letter = '';
    part1.split('').forEach(chr => {
        if (part2.includes(chr)) {
            letter = chr;
        }
    });
    const charValue = getCharValue(letter);
    //console.log(row, part1, part2, letter, charValue);
    return charValue;
})
...
for (let i=0; i<rows.length;i=i+3) {
    let letter = '';
    rows[i].split('').forEach(chr => {
        if (rows[i+1].split('').includes(chr) && rows[i+2].split('').includes(chr)) {
            letter = chr;
        }
    });
